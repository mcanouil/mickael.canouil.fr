...
I wanted to know how much of one Quarto can carry on its own.
...
2. The two template partials that build the pages, and the filter that numbers the floats by chapter.
3. The fonts travel with the extension, so the PDF does not depend on what is installed.
4. The theme, and the filter that inlines the figures.
...
#let node(body, detail: none, stroke: box-stroke) = rect(
  ...
  #align(center + horizon)[
    #stack(spacing: 4pt,
      ...
      ..if detail == none { () } else { (text(size: 7pt, fill: muted)[#detail],) },
    )
  ]
]
...
#layout(avail => {
  ...
  grid(
    ...
    rows: (row, 12pt, row, 12pt, row),
    align: horizon,
    grid.cell(rowspan: 5, stack(spacing: 12pt,
    ...
    grid.cell(rowspan: 5, arr),
    grid.cell(rowspan: 5, node(
      [Quarto], detail: "with the nordvale/clinical extension", stroke: teal + 1.2pt,
    )),
    grid.cell(rowspan: 5, arr),
    grid.cell(rowspan: 5, stack(spacing: 12pt,
...
![The **Typst** PDF](assets/table.png){fig-alt="Page 15 of the PDF. A numbered heading reads 7 Primary and secondary endpoints, followed by Table 7.1 with a dark teal header row, a source note under it, and a Kaplan-Meier figure with a risk table." fig-align="center" .hero-art}

![The **HTML** site](assets/html-site.png){fig-alt="The same chapter on the HTML site. A left sidebar lists the chapters and appendices, and the main column shows Table 7.1 with the same teal header row and the same Kaplan-Meier figure." fig-align="center" .hero-art}
:::
...
I reported that as [quarto-cli#14772](https://github.com/quarto-dev/quarto-cli/issues/14772), and the fix proposed in [#14773](https://github.com/quarto-dev/quarto-cli/pull/14773) targets Quarto 1.11.
...
That flag then drives the notice on the cover and the background of every page.
