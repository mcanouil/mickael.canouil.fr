...
The guideline stops at the group, and the digits after it are the sponsor's own sequence, which is why the per-protocol version of 14.2.1 is 14.2.1.1.
...
Quarto builds a float number from the chapter and a running count, and nothing in the document can set it: a custom cross-reference type names the prefix, not the number.
...
Quarto writes its cross-reference index before any extension filter runs, and a book fills every reference from that index after Pandoc, so a filter never reaches those numbers.
