...
The short answer: not that hard, but there is one non-obvious gotcha.
...
3. Controls which names appear on the Reference page and in what order — names not listed here are hidden from the public API.
...
3. great-docs writes to `great-docs/_site` by default; this renames it to `docs`, which is the path the Starlette mount expects.
        ...
        ui.layout_sidebar(
            ui.sidebar(
                ui.input_selectize("species", "Species", choices=_SPECIES,
                                   selected=_SPECIES, multiple=True), # <1>
            ),
...
3. The Shiny app is mounted last — Starlette tries routes in order, so the more-specific `/docs` prefix always wins before the catch-all `/`.
...
[uvicorn](https://www.uvicorn.org/) accepts any ASGI application, so the development workflow does not change.
