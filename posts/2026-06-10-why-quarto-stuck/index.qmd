...
Even the format names are more intuitive: `html`, `pdf`, `revealjs`, `website`, and `book` are easier to remember than the R Markdown equivalents (_also way less typing_).
They are consistent across formats too, so I do not have to remember that `html_document` is for HTML but `pdf_document` is for PDF.
...
None of this is magic, it is less surprising, and after years of small surprises that matters.
...
The look and feel by default is, in my opinion, better, especially for HTML.
...
I cannot wait.
Backwards compatibility is one of the main goals, but some breaking changes are likely to happen in favour of a better experience.
