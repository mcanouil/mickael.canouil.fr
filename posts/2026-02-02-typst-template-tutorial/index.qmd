...
local function extract_card(div)
  ...
  if #content_blocks > 0 then
    card.content = pandoc.utils.stringify(content_blocks) -- <5>
  end
...
1. [`pandoc.List`](https://pandoc.org/lua-filters.html#type-list) provides list methods like `:insert()`.
2. State machine flag tracks position relative to the horizontal rule separator.
3. Only first header becomes title; subsequent headers remain in content.
4. `---` in markdown becomes a `HorizontalRule` AST element.
5. Flattens rich content to plain text for the Typst function.
...
local function process_div(div, config)
  ...
  for _, card in ipairs(cards) do
    ...
    table.insert(card_items, '(' .. table.concat(card_parts, ', ') .. ')') -- <5>
  end

  -- Build final Typst function call
  local typst_code = string.format( -- <6>
    '#mcanouil-card-grid(\n  (%s),\n  columns: %d\n)',
...
5. Each card becomes a Typst dictionary literal: `(title: "...", content: "...")`.
6. Final output is a Typst array of dictionaries passed to the grid function.
