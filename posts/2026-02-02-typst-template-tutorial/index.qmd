...
pandoc.RawBlock('typst', '#my-function[content]')

-- For inline elements (spans, text)
pandoc.RawInline('typst', '#my-function[content]')
```
...
The quarto-mcanouil extension uses a wrapper module to handle attribute conversion and function call generation:
...
#let mcanouil-badge(content, ..args) = {
  render-badge(
    content,
    colours: mcanouil-colours(mode: effective-brand-mode),
    ..args
  )
}
```
...
#let get-panel-colours(style, colours) = {
  if style == "info" {
    ...
    let text-colour = get-wcag-compliant-text-colour(bg, colours, min-ratio: 4.5)
    (background: bg, border: info-colour, title: text-colour, content: text-colour)
...
The card extraction function demonstrates several patterns for parsing nested content:
...
local function extract_card(div)
  ...
  -- Process blocks to separate title, content, footer
...
- **State machine**: The `in_footer` flag tracks whether we are before or after the horizontal rule.
- **First-match extraction**: The title is taken from the first header encountered.
- **List accumulation**: Content and footer blocks are collected separately.
...
local function process_div(div, config)
  ...
  local columns = div.attributes.columns
    and tonumber(div.attributes.columns) or 3
  ...
  for _, card in ipairs(cards) do
    ...
    if card.title then
      table.insert(card_parts,
        string.format('title: %s', typst_utils.typst_value(card.title)))
    end
...
#let render-card-grid(cards, colours, columns: 3, gutter: 1em) = {
  ...
  let cells = cards.map(card => {
    let card-colours = get-card-colours(card.at("style", default: "subtle"), colours)
...
The configuration system enables this through metadata-driven element mappings.
...
local function get_builtin_mappings()
  return {
    div = {
      ['highlight'] = { wrapper = 'mcanouil-highlight', arguments = false },
      ['value-box'] = { wrapper = 'mcanouil-value-box', arguments = true },
...
local function parse_and_validate_config(config, class)
  ...
  if type(config) == 'string' or (config_str and not string.find(config_str, '[%{%[]')) then
    if utils.is_empty(config_str) then
      utils.log_warning(EXTENSION_NAME, 'Empty function name for class "' .. class .. '"')
      return nil
    end
...
local function build_attribute_string(attrs)
  ...
  for key, value in pairs(attrs) do
    table.insert(attr_items,
      string.format('%s: %s', key, typst_utils.typst_value(value)))
  end
...
return {
  ['my-shortcode'] = typst_utils.create_shortcode_handler(
    'my-typst-function',
    { color = 'colour' }  -- Map 'color' to 'colour'
  )
}
```
...
```{.lua filename="typst-markdown.lua"}
return {
  { Meta = Meta },                                    -- First pass: load configuration
  { Div = Div, Span = Span, Table = Table, Image = Image }  -- Second pass: transform elements
}
```
...
quarto.log.output('Generated Typst code:', typst_code)
```
...
#let mcanouil-quote-card(content, ..args) = {
  render-quote-card(
    content,
    colours: mcanouil-colours(mode: effective-brand-mode),
    ..args
