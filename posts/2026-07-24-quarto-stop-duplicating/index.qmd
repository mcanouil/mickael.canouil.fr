...
Under a custom `mcanouil-typst` format, only `mcanouil-typst:` matches, not `typst:`.
The built-in PDF format is targeted with `pdf:`, since that is the format name Quarto requests, not the writer it happens to use underneath.
...
The table gets a width in the HTML handout and a smaller font on the deck, from a single wrapper.
Typst ignores `max-width`, so the PDF falls back to its own table sizing.
...
![](assets/output/demo-slides.html){
  ...
  .slide-deck
  loading="lazy"
  title="Reveal.js slide deck rendered from the demo source"
}
...
- `cascade` does not hoist a `---` that sits inside a `.panel-tabset` div, so a slide break there is left alone rather than split into a continuation slide.
