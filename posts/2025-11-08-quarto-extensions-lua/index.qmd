...
However, with this power comes opportunity.
...
Drawing from patterns established in the Quarto core team's extensions and emerging community standards, this guide explores the essential practices that help transform initial working extensions into well-crafted, robust solutions.

**A note on perspective**: This guide presents opinionated recommendations based on patterns observed in successful Quarto extensions and general software development best practices.
...
To prevent conflicts with Quarto CLI options and other extensions, consider scoping your extension's configuration parameters under your extension's name in the document metadata.  
...
local function get_extension_options(meta)
  ...
  local extension_config = extensions_config[EXTENSION_NAME]
...
Consider loading modules and dependencies only when actually needed:

```{.lua filename="Lua"}
-- Don't load heavy modules at the top level
local colour_utils = nil

local function get_colour_utils()
  if not colour_utils then
    local path = quarto.utils.resolve_path("_modules/colour.lua")
    local clean_path = path:gsub("%.lua$", "")
    colour_utils = require(clean_path)
  end
  return colour_utils
end
```
...
Setting up your development environment with proper tooling can be very helpful.  
The `.luarc.json` file in your project root enables type checking and autocomplete^[See [Lua in VS Code](https://quarto.org/docs/extensions/lua.html#lua-in-vs-code) documentation for more details.]:
...
```json
{
  ...
  "Lua.workspace.library": [
    "/Users/mcanouil/Projects/quarto-dev/quarto-cli/src/resources/lua-types"
  ],
  "Lua.runtime.plugin": "/Users/mcanouil/Projects/quarto-dev/quarto-cli/src/resources/lua-plugin/plugin.lua",
...
Including working examples in your extension is highly recommended.
The `example.qmd` file serves multiple purposes:

- **Documentation**: Shows users how to use your extension.
- **Testing**: Provides a quick way to verify functionality.
- **Development**: Enables rapid iteration with `quarto preview`.
...
This document demonstrates the extension:
...
The practices outlined in this guide represent one perspective on extension development, drawn from patterns observed in successful extensions and general software development principles.
...
Creating releases not only provides users with stable versions to reference but also enables powerful tooling like Quarto Wizard's update feature to automatically manage extension lifecycles.
