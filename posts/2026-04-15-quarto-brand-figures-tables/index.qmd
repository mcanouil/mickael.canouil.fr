...
This post demonstrates one particular approach: reading the resolved brand configuration from `QUARTO_EXECUTE_INFO` at render time, making it independent of external packages and portable across languages, with full font registration support.
...
The brand configuration lives at `format.render.brand` in the JSON, with separate entries for each colour mode (`light`, `dark`).
...
The brand configuration under `format.render.brand` contains one entry per colour mode.
...
An SVG file referenced via an `<img>` tag is treated as an isolated document by the browser.
It cannot access the page's stylesheets or `@import` rules, so any `@font-face` declarations inside the SVG that reference external URLs are silently ignored.
...
The `theme_brand()` function extracts colours and fonts from the brand configuration and builds a complete `ggplot2` or plotnine theme.
...
Quarto supports light and dark colour modes, and the brand configuration includes separate colour definitions for each.
...
Every helper reads from the same `QUARTO_EXECUTE_INFO` source, so switching to a different brand configuration is a matter of changing your `_brand.yml` file; no code changes are needed.
