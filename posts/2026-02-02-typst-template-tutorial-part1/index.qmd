...
2. Wrapper delegates to `simple-badge`, which uses `get-badge-colour` internally for colour lookup.

The Lua filter generates `#my-badge(colour: "success")[Completed]`.
At render time, `my-badge` delegates to `simple-badge` which resolves the colour internally via `get-badge-colour`.
For block components like panels, the wrapper pattern becomes more important as it injects the document's colour scheme.

:::: {.highlight}

**Wrapper functions** bridge the gap between parse-time code generation (Lua) and render-time colour resolution (Typst).
For block components, they inject runtime context (such as the colour scheme) that Lua cannot know in advance.
