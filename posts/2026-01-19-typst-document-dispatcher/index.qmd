...
While Quarto extensions can contribute multiple variants of the same base format[^variants], this approach requires either sharing metadata across all contributed elements or duplicating it for each variant.

The dispatcher pattern offers an alternative: a single format entry point with document-type routing handled entirely in Typst code.
This centralises shared logic and avoids metadata duplication.

[^variants]: Format variants use the `+` syntax in `_extension.yml`:
...
- Always gives the dispatcher a valid string.
...
2. **Document type templates** define the rendering functions (`render-report`, `render-letter`, `render-cv`).
3. **Dispatcher** comes last because it references the rendering functions.
