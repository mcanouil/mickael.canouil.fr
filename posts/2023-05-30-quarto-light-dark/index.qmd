...
1. Set the `dev` option to use `svglite` for `light` mode and `darksvglite` for `dark` mode and `fig.ext` to set the generated images extensions.
   ...
   ```
   1. We use `svglite` to generate SVG images for `light` mode and `darksvglite` for `dark` mode.
   2. We use `.light.svg` as the extension for `light` image and `.dark.svg` for `dark` image.
