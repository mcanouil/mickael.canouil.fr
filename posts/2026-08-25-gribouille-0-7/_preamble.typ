...
// `theme-brand` takes its colours from the brand, not from the document, so it
// is not wrapped like the others. It reads `mode` instead: `auto` picks the
// light or the dark side of the brand's variants from the page colour, so a
// branded figure still follows the light / dark toggle of the site.
#let _theme_brand = theme-brand

#let theme-brand(brand, mode: auto, ..fields) = {
  let resolved = if mode != auto {
    mode
  } else if _typst_render_background != none {
    if luma(_typst_render_background).components().first() < 50% {
      "dark"
    } else {
      "light"
    }
  ...
  _theme_brand(brand, mode: resolved, ..fields)
}
