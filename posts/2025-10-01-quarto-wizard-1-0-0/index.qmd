---
title: "{{< meta quarto-wizard >}} Quarto Wizard 1.0.0: Democratising Quarto Extension Management"
pagetitle: "Quarto Wizard 1.0.0: Democratising Quarto Extension Management"
description: |
...
![](featured.png){
  .hero-banner
  fig-align="center"
  fig-alt="Cartoon dog wizard wearing blue hat with red band holding magic wand creating HTML and CSS code scrolls in starry night scene"
  width="600px"
}

I'm absolutely thrilled to announce **{{< meta quarto-wizard >}} Quarto Wizard 1.0.0**, a groundbreaking extension for Visual Studio Code and Positron that transforms how you interact with the {{< quarto >}} Quarto ecosystem.
...
- Via VS Code or Positron Extensions view:
  ...
  ![Extensions View: Marketplace](assets/media/extensions-marketplace-dark.png){.dark-content width="500px"}![Extensions View: Marketplace](assets/media/extensions-marketplace-light.png){.light-content width="500px"}
...
- Via the command line:
  ...
  Be sure to execute the command *Shell Command: Install 'code' command in PATH* from Visual Studio Code's Command Palette ({{< kbd Ctrl-Shift-P >}} or {{< kbd Cmd-Shift-P >}} on macOS) if you haven't done so already.
  ...
  Be sure to execute the command *Shell Command: Install 'positron' command in PATH* from Positron's Command Palette ({{< kbd Ctrl-Shift-P >}} or {{< kbd Cmd-Shift-P >}} on macOS) if you haven't done so already.
...
At the heart of **Quarto Wizard** lies the [Quarto Extensions directory](https://m.canouil.dev/quarto-extensions/), a comprehensive listing I maintain that catalogues extensions from across the entire Quarto ecosystem.
...
The integration between **Quarto Wizard** and this directory creates a seamless experience: instead of manually searching GitHub repositories or memorising command-line syntax, you can browse hundreds of extensions with detailed information at your fingertips.
This transforms extension discovery from a treasure hunt into a curated shopping experience.

## The extension management revolution
...
The solution is **multi-modal installation**: you can now install extensions through multiple pathways that suit your workflow.

Visit any extension on [m.canouil.dev/quarto-extensions](https://m.canouil.dev/quarto-extensions/) and you'll find installation options for every preference: click the **Quarto Wizard** button to install directly through VS Code or Positron, copy terminal commands for traditional installation, or use the dedicated IDE buttons for VSCodium.
...
# The new ways:
# - Click "Install" on the website
# - Use Quarto Wizard in your IDE
# - Browse and filter by popularity, updates, or categories
```
...
What makes this particularly powerful is **intelligent extension management**.
**Quarto Wizard** tracks which extensions were installed through its interface by adding `source` metadata to the `_extensions.yml` file, enabling seamless updates and removals.[^1]
...
## Template discovery made effortless

Beyond extension management, I've designed **Quarto Wizard** to excel at **template discovery and deployment**.
The web directory at [m.canouil.dev/quarto-extensions](https://m.canouil.dev/quarto-extensions/) enhances this with powerful filtering options: you can sort by recently updated, filter by popularity, browse by categories (*i.e.*, Shortcodes, Filters, Formats, Projects, Reveal.js Plugins), or search for specific functionality.
...
```{.markdown}
# Access via Command Palette in VS Code or Positron
...
## Streamlined document creation workflows

I've introduced the "Quarto Reproducible Document" feature to generate starter documents optimised for different content types: whether you're writing a bug report, feature request, or general content piece to share with the community.
These templates include pre-configured YAML front matter with computational settings (*i.e.*, R, Python, and Julia), code blocks, and example content to get you started quickly.

This addresses a common pain point where users spend considerable time setting up proper document structure, YAML configuration, and initial code blocks.
**Quarto Wizard** handles these details automatically whilst maintaining flexibility for customisation.
...
The extension includes comprehensive **logging capabilities** that work identically in both environments.
When installation issues arise, detailed output logs help identify and resolve problems quickly.
This is particularly valuable when working with complex extension dependencies or network connectivity challenges.

## Advanced extension management capabilities

For power users, I've built in **sophisticated extension management features**.
...
- **Command-line intimidation**: Users who prefer graphical interfaces no longer need to memorise terminal commands or navigate complex installation procedures.
