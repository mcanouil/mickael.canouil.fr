...
This tutorial introduces the dual-layer architecture for building Quarto Typst extensions: Lua filters intercept markdown elements at parse time, Typst functions render styled output at render time.

- **Badges**: inline pill-shaped indicators built with `pandoc.RawInline` and a simple Typst `box` function.
- **Panels**: block components that wrap rich nested content with opening and closing Typst calls via `pandoc.RawBlock`.
- **Template variables**: `effective-brand-mode` bridges Lua parse-time and Typst render-time for colour scheme selection.
...
In professional environments, document presentation is more than aesthetic.
It reflects organisational identity and credibility.
You might produce academic papers, corporate reports, client proposals, or technical documentation.
Consistent branding turns these different documents into one recognisable, trustworthy portfolio.

Yet keeping this consistency by hand is difficult.
...
**Reproducible branding** through code eliminates manual styling errors, so every document reflects your organisation's identity.
...
When paired with Quarto's publishing ecosystem, Typst becomes a foundation for branded document production.

## Prior Art: Getting Started with Typst and Quarto

Before covering advanced techniques, a few excellent foundational resources deserve a mention.
David Keyes wrote an introduction to using Typst with Quarto at [R for the Rest of Us](https://rfortherestofus.com/2025/11/quarto-typst-pdf).
...
[Part 2](/posts/2026-03-05-typst-template-tutorial-part2/) explores advanced patterns including handler factories, configuration systems, and WCAG-compliant styling.
...
Without a Lua filter, Pandoc's default Typst writer drops the `.badge` class and outputs plain text with no styling.
Your Typst template never sees the class at all.
...
Before covering component implementation, let us set out the minimal file organisation needed for a Typst extension.
...
[Part 2](/posts/2026-03-05-typst-template-tutorial-part2/) covers those architectural patterns.
...
[Part 2](/posts/2026-03-05-typst-template-tutorial-part2/) replaces this with an implementation that also handles actual Lua booleans, `nil`, and Typst measurement units like `1em` or `2.5pt`.
...
- `quarto.brand.get_typography(mode, name)`: returns a table of typography options such as family, size, and weight for the given element.
...
- **`div.content`**: An array of block elements, such as paragraphs and lists.
...
2. Empty dictionary `(:)` as default; the parameter is declared now to keep the function signature stable, so [Part 2](/posts/2026-03-05-typst-template-tutorial-part2/) can add colour-aware logic without changing any call sites. The wrapper function in `typst-show.typ` injects the actual colour scheme, but this simplified Part 1 version of `get-panel-colours` ignores it.
3. Returns a [dictionary](https://typst.app/docs/reference/foundations/dictionary/) with background, border, title, and content colours based purely on the `style` name; [Part 2](/posts/2026-03-05-typst-template-tutorial-part2/) upgrades this to also use the injected `colours` for theme-aware text.
...
Unlike badges, where the Lua filter passes `effective-brand-mode` directly to `simple-badge`, panels need a richer colour dictionary.
...
2. Forwards all other named arguments, such as `title` and `style`, unchanged.

The Lua filter generates `#my-panel(style: "info")[...]`, and at render time `my-panel` resolves the colour scheme before delegating to `render-panel`.
...
5. The `config` table matches the shape used in [Part 2](/posts/2026-03-05-typst-template-tutorial-part2/)'s configuration system: `wrapper` is the Typst function name, `arguments = true` ensures the parameter list is always generated.
...
[Part 2](/posts/2026-03-05-typst-template-tutorial-part2/) builds on these foundations with advanced patterns:
...
- [Part 2: Advanced Patterns](/posts/2026-03-05-typst-template-tutorial-part2/)
