...
Version 0.1.0 is released on [Typst Universe](https://typst.app/universe/package/gribouille) today, modelled closely on `ggplot2`, drawing on top of [`cetz`](https://cetz-package.github.io/), and shipped with a documentation site that is itself a Quarto project rendered by the [`quarto-typst-render`](https://github.com/mcanouil/quarto-typst-render) extension.
...
- 45 geoms, 18 stats, 65+ scales, multiple themes, full facet support.
...
`cetz` already gives Typst a robust low-level drawing API.
What was missing was the layer in between: a declarative grammar that turns a dataset, a mapping, and a few layers into a publication-quality figure without you having to handle ticks, breaks, legends, or coordinate transforms by hand.
That is exactly the gap Gribouille fills.

The pay-off is a single toolchain.
A Gribouille figure (`kind: "gribouille-plot"`) shares the document's fonts, the document's colours, and the document's geometry.
...
This release would not exist without Hadley Wickham's original vision, nor without the people carrying it forward today, in particular Thomas Lin Pedersen and Teun van den Brand, and the wider community of contributors who keep shaping the package to the recent v4 release.
...
Where the two part ways, it is because Typst is a different language, with no lazy evaluation, no plot object you can mutate after the fact, no `ggsave()` output pipeline, and no `vars()` quoting.
...
//| alt: "Scatter of body mass against flipper length, points shaped by species."
#plot(
  data: penguins,
  mapping: aes(
    x: "flipper-len",
...
[`format-comma()`](https://m.canouil.dev/gribouille/reference/helpers/format-comma.html), passed to `scale-y-continuous(labels: ...)`, turns `5000` into `5,000` on the y-axis; sibling helpers cover percent, currency, and scientific notation.
A single `species-colours` dictionary feeds both [`scale-colour-discrete()`](https://m.canouil.dev/gribouille/reference/scales/scale-colour-discrete.html) (`limits:` + `palette:`) and the subtitle: the `typst({...})` constructor iterates its pairs to print each species name in its own colour, on a second line below the linear-fit formula.
[`element-typst()`](https://m.canouil.dev/gribouille/reference/themes/element-typst.html) covers the simpler case where the subtitle is a plain string carrying Typst markup like `$hat(y) = beta_0 + beta_1 dot.c x$`.
...
//| alt: "Polished version of the faceted scatter with title block, axis labels, formatted ticks, a minimal theme, and a subtitle whose second line lists the species names in their own colours."
#let species-colours = (
  Adelie:    rgb("#0072B2"),
...
#plot(
  ...
  labs: labs(
    ...
    subtitle: typst({
      [Linear fit $hat(y) = beta_0 + beta_1 dot.c x$ on every island \ ]
      species-colours
        .pairs()
...
Nothing else changes; the data has simply crossed a language boundary.

```{typst}
//| echo: true
//| output-filename: "polars-summary.svg"
//| alt: "Bar chart of mean body mass per species, computed in Polars and rendered with Gribouille."
#let cb = (
  rgb("#0072B2"),
  rgb("#D55E00"),
  rgb("#009E73"),
...
#plot(
  ...
  labs: labs(
    ...
    subtitle: "Polars on the inside, Gribouille on the outside",
    x: "Species",
    y: "Body mass (g)",
    fill: "Species",
  ),
...
- The Lua filter runs at the `pre-quarto` stage, which means `typst_define` is in scope for every `{typst}` block from that point in the document onward.
...
A handful of `ggplot2`/`plotnine` features are deferred or left out by design.
...
- Maps and `sf` geometries are currently out of scope.
