...
`cetz` already gives Typst a low-level drawing API, but the declarative layer that turns a dataset, a mapping, and a few layers into a publication-quality figure was missing.
...
Give a plot an `alt:` description and Gribouille wraps the result in a Typst `figure` element with `kind: "gribouille-plot"`, so you can attach show rules, captions, or cross-references in one place.
Every plot shares the document's fonts, palette, and geometry, whether or not you add that description.
...
**Many plots, one legend.** `compose()` hoists the shared aesthetics for you.
...
::: {.callout-note}
You only need [`compose()`](https://m.canouil.dev/gribouille/reference/core/compose.html) when you want the shared legend hoisted.
A rendered plot is ordinary Typst content, so the base [`grid`](https://typst.app/docs/reference/layout/grid/) and [`stack`](https://typst.app/docs/reference/layout/stack/) functions arrange several plots just as well when each keeps its own legend.
:::

## Drive Gribouille from any computation

`Typst Render` 0.13.1 adds a small helper called `typst_define()`, available from both R and Python.
...
**Compute anywhere, render in Typst.** `typst_define()` is the bridge.
...
- [`geom-dotplot()`](https://m.canouil.dev/gribouille/reference/geoms/geom-dotplot.html) uses histogram-style binning only; data-driven dot-density binning is deferred.
...
**Small, focused, and modelled on the library that has shaped a generation of data visualisation.**
