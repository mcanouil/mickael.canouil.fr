...
And a scale `limits` or `expand` accepts `auto` on one side, keeping the trained value for the other.
...
## Theme, Position, and Scale Fixes
...
- Explicit continuous `breaks` expand the axis range so the requested ticks stay visible, unless a side is pinned by explicit `limits`.
