...
0.6.0 replaces it with the extended Wilkinson search [@talbot_extension_2010], which scores candidate tick sequences on three things at once: how simple the numbers are, how well they cover the data, and how close the count lands to the target.
...
The reference pages had a real hole in them.
...
Footer links were painted the page colour in light mode, which made them invisible, and the rule above the footer was 8% black on dark paper, which came to much the same thing.
The copy button on a one-line code block hung past its bottom border.
The navbar is dark in both colour schemes now, with scheme-aware scrollbars, a skip link, and a visible focus outline, and the tabset panel border no longer vanishes into the dark background.
On a narrow screen, opening a navbar menu overlays the page instead of pushing the other items down, and the colour-scheme toggle and the search button sit together at the right edge with the brand centred.
...
One change is worth a line of its own.
