...
This tutorial was written and tested with **Quarto CLI 1.9.29 (pre-release)**.
...
This tutorial covers advanced patterns for production-quality Quarto Typst extensions.
Read [Part 1](/posts/2026-02-27-typst-template-tutorial-part1/) first for the foundational concepts.
...
- **WCAG-compliant badges**: iterative contrast adjustment produces accessible colour combinations.
...
When building your own extension, you can choose any descriptive name, as long as it matches the filename declared under `filters:` in `_extension.yml`.
...
When `create_wrapped_handler(true)` returns a new function, that function carries `should_extract_title = true` inside it.
Every call to the returned handler then uses the correct flag automatically, without passing it again.
This is necessary because the filter loop (shown below in "Using Factories") calls each handler as `DIV_FACTORY_HANDLERS[class](div, class_config)`, passing only the div element and the class config.
The flag itself is never passed.
...
1. Spans can carry multiple classes, for example `[text]{.badge .highlight}`. `span.classes` preserves declaration order, left to right as written in the markdown, so iterating finds the first mapped class. `return` exits immediately, so only one handler fires per span.
...
- **No handler (default)**: Simple wrapper around content; add a mapping in document YAML.
...
Users can add their own components or override built-in ones without modifying the extension's source code.
...
Both `function:` and `wrapper:` are accepted as the key name in the detailed format, and are true aliases.
`wrapper:` matches the internal Lua variable name, whilst `function:` is more descriptive in a YAML context, so `function:` is preferred.
...
Two helpers keep the logic simple:
