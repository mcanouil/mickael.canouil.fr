...
image: featured.png
image-alt: |
  Docker logo with a whale and containers, and Quarto logo with text inside the whale.
...
For example, I create a website using the Quarto CLI and add a code cell to the `about.qmd` file that uses the `palmerpenguins` R package.  
...
Here, I am using "build arguments" (_i.e._, `ARGS`) to specify the [Quarto](https://quarto.org/), the [rig](https://github.com/r-lib/rig) and the R versions.  
...
Again, I am using "build arguments" (_i.e._, `ARGS`) to specify the [Quarto](https://quarto.org/), the [rig](https://github.com/r-lib/rig) and the R versions.  
