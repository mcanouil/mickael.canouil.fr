...
3.4 follows straight after 3.3, and it is about the two features above, so the list below holds both releases.
...
- The search for Quarto projects no longer collects search processes.
  It applies your `files.exclude` and `search.exclude` settings, and it stops when a newer search replaces it.
- The preview reads a document again when that document takes the name of an untitled document you closed.
  It used to show you the blocks of the one before.
- A fenced block that a tab indents inside a blockquote is found, and a body line keeps the indent you wrote.
- A YAML diagnostic points at the key or the value it is about, rather than at the whole line, and an option written in flow style is checked as well.
- The Lua validator corrects two checks, accepts a name written with a hyphen where the schema declares it with an underscore, and reports a spelling that two attributes both declare.
  That last one used to leave the choice to the validator.

The changelog holds the full list, for [3.3](https://m.canouil.dev/quarto-wizard/changelog.html#version-3-3) and for [3.4](https://m.canouil.dev/quarto-wizard/changelog.html#version-3-4).
