...
The [changelog](https://m.canouil.dev/quarto-wizard/changelog.html#version-3-3) holds the full list.

## What 3.4 Adds

3.4 follows straight after, and it is about the two features above.
The preview reads a document again when that document takes the name of an untitled document you closed, so it no longer shows you the blocks of the one before.
It also finds a fenced block that a tab indents inside a blockquote.

On the schema side, a YAML diagnostic now points at the key or the value it is about, rather than at the whole line, and an option written in flow style is checked as well.
The Lua validator corrects two checks, accepts a name written with a hyphen where the schema declares it with an underscore, and reports a spelling that two attributes both declare, which used to leave the choice to the validator.

The [3.4 changelog](https://m.canouil.dev/quarto-wizard/changelog.html#version-3-4) holds that list.
