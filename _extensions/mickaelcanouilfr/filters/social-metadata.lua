--- mickaelcanouilfr - Social Metadata Filter
--- @module "social-metadata"
--- @note Copied from the `atelier` extension
--- (https://github.com/mcanouil/quarto-atelier); kept in step with it, with
--- the extension name changed and blog posts typed as `article`.
--- @license MIT
...
local EXTENSION_NAME = 'mickaelcanouilfr'
...
--- Directory holding the blog posts. Pages below it are long-form writing and
--- take `og:type: article`; everything else is `website`.
--- @type string
local POSTS_DIRECTORY = 'posts/'
...
--- The Open Graph type for the page.
--- @param relative_output string The output path relative to the project root
--- @return string
local function og_type(relative_output)
  if relative_output:sub(1, #POSTS_DIRECTORY) == POSTS_DIRECTORY then
    return 'article'
  end
  return 'website'
end
...
local function social_metadata(meta)
  ...
  local tags = {
    string.format('<meta property="og:type" content="%s">', og_type(relative_output)),
  }
